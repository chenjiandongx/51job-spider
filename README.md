...
提取了所有的职位要求，进行分词统计，清理没意义的词，统一英文字符，如 Python 和python 不区分大小。  
...
最多的三个（开发,2100）（熟悉，1842）（经验，1268），熟悉其实也就是相当于有经验了。  
...
然后对职位进行分析，这个问题让我头可疼可疼了呢，因为没有一个规范，导致招聘信息上的职位写的是各式各样，举个例子吧。Python 高级开发工程师和高级 Python 开发工程师是一个意思吧。Python 开发工程师，python 开发工程师，PYTHON 工程师这三个是一个意思吧。有的甚至写着 Python 攻城师？？？ 这样就造成了很难进行统计，找不到好办法，只能先进行词频统计，然后剩下的再进行手动归类。最终被我归为 20 个类别。
